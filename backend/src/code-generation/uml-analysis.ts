...
export const normalizeAndValidateUml = (snapshot: Record<string, unknown>): UmlAnalysis => {
  ...
  rawElements.forEach((raw: any, index) => {
    ...
    if (isEnum) {
      ...
      elements.push({ id, name, kind: 'enum', attributes: [], structuredAttributes: [], literals, ...(raw.metadata ? { metadata: raw.metadata } : {}) }); return;
    }
    ...
    elements.push({ id, name, kind, attributes, structuredAttributes, literals: [], ...(persistible ? { persistible } : {}), ...(raw.metadata ? { metadata: raw.metadata } : {}) });
  });
