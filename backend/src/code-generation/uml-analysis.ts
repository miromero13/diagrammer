...
const normalizedName = (name: string) => name.replace(/([a-z])([A-Z])/g, '$1_$2').toLowerCase();
const physicalTableName = (name: string) => {
  const normalized = normalizedName(name);
  return normalized === 'user' ? 'users' : normalized;
};
const columnName = (name: string) => normalizedName(name);
...
export const normalizeAndValidateUml = (snapshot: Record<string, unknown>): UmlAnalysis => {
  ...
  const excludedElements = elements.filter((e) => e.kind === 'interface' || (e.kind === 'abstract' && !inheritedIds.has(e.id)) || (e.kind === 'class' && /service$/i.test(e.name))).map((e) => ({ id: e.id, name: e.name, kind: e.kind as 'class' | 'interface' | 'abstract', reason: e.kind === 'interface' ? 'La interfaz no es persistible; se conserva como elemento técnico excluido.' : 'La clase técnica *Service se excluye; se conserva como elemento técnico excluido.' }));
   ...
   const physicalTableNames = new Set<string>();
    tableElements.forEach((element) => { const name = physicalTableName(element.name); if (physicalTableNames.has(name)) errors.push(`Nombre físico de tabla duplicado: ${name}`); physicalTableNames.add(name); });
  ...
  const tables = new Map(tableElements.map((e) => [e.id, { id: physicalTableName(e.name), name: physicalTableName(e.name), sourceElementId: e.id, columns: columnsFor(e) } as RelationalTable]));
